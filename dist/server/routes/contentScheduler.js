...
async function postToThreads(text, media, auth) {
    ...
    try {
        ...
        const containerResponse = await fetch(`https://graph.threads.net/v1.0/${auth.userId}/threads`, {
            method: 'POST',
            headers: {
        ...
        const publishResponse = await fetch(`https://graph.threads.net/v1.0/${auth.userId}/threads_publish`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${auth.accessToken}`,
                'Content-Type': 'application/json'
